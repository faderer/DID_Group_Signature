...
sign0 -1`)
   ...
   fmt.Println(g1.Bytes())
   fmt.Println(g2.Bytes())
   fmt.Printf("g1:%02x \n",priv.g1.Bytes())
   fmt.Printf("h:%02x \n",priv.h.Bytes())
   fmt.Printf("u:%02x \n",priv.u.Bytes())
   fmt.Printf("v:%02x \n",priv.v.Bytes())
  ...
  combine1:=append(append(append(append(append(sig.c1.Bytes(),sig.c2.Bytes()...),sig.c3.Bytes()...),sig.t1.Bytes()...),sig.t2.Bytes()...),sig.t3.Bytes()...)
  combine2:=append(append(append(append(append(combine1,sig.c.Bytes()...),sig.salpha.Bytes()...),sig.sbeta.Bytes()...),sig.sa.Bytes()...),sig.sx.Bytes()...)
  combine:=append(append(append(combine2,sig.sdelta1.Bytes()...),sig.sdelta2.Bytes()...),member.h_.Bytes()...)
  fmt.Printf("combine:%02x \n",combine)
  fmt.Println(len(combine))
  fmt.Println(len(sig.sa.Bytes()))
   //sd:=pairing.NewZr().SetBytes([]byte{103, 81, 121, 195, 87, 85, 40, 30, 66, 241, 133, 50, 21, 209, 30, 192, 185, 40, 27, 126})
   //fmt.Printf("sd:%02x \n",sd.Bytes())
   //verify    signature
